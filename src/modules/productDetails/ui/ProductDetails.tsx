...
export const ProductDetails = () => {
  ...
  const renderContent = () => {
    const merch = merchants?.merchants

    if (!merch?.length) return <span>no merchants</span>

    return merch.map((item) => {
      const currency = getCurrency(item.currency)
      const ratingColor = getRatingColor(item.rating)
      const formattedDate = formatDate(new Date(item.delivery))

      return (
        <div key={item.name} className='mt-2 border rounded-xl p-2 first:mt-0'>
      ...
      <Card className='ring-2 ring-blue-400 h-[600px] flex flex-col'>
        ...
        <CardContent className='flex-1 overflow-y-auto'>
          ...
          <div className='mt-4 space-y-2'>
            {specs?.characteristics?.map((item) => (
              <div
                key={item.value}
